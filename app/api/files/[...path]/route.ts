...
import { didFileWatchSnapshotChange, type FileWatchSnapshot } from "@/lib/file-watch-state";
    ...
    for (const file of files) {
      ...
      try {
        if (conflictSet.has(file.name)) {
          writeFileAtomicSync(destination, bytes);
        } else {
...
export async function GET(
  ...
  try {
    ...
    if (type === "watch") {
      ...
      let lastSnapshot: FileWatchSnapshot | null = { mtimeMs: stat.mtimeMs, size: stat.size };
          ...
          try {
            watcher = fs.watch(filePath, () => {
              try {
                ...
                const nextSnapshot = { mtimeMs: s.mtimeMs, size: s.size };
                if (!didFileWatchSnapshotChange(lastSnapshot, nextSnapshot)) return;
                lastSnapshot = nextSnapshot;
                send("change", { mtime: s.mtime.toISOString(), size: s.size });
              } catch {
                if (!didFileWatchSnapshotChange(lastSnapshot, null)) return;
                lastSnapshot = null;
